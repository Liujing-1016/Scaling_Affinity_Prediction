...
Output:
- all_predictions_xgb_1.parquet: Predicted pIC50 for all targets in `train_target_1.parquet` (float32).
...
train_file = "../data/processed/filter/train_split_5.parquet"
test_smiles = "../data/raw_filter_20/test_f_smiles.csv"
model_dir = "../data/models/XGBoost"
output_file = "../data/results/predictions_XGB_filter/predictions_xgbf_5.parquet"
...
if os.path.exists(output_file):
    ...
    # Determine the starting index based on completed targets
    completed_targets = [col for col in targets if col in all_predictions_df.columns and col != 'smiles']
    start_index = len(completed_targets)
    print(f"Resuming from target index {start_index}, {completed_targets[-1] if completed_targets else 'none'} was the last completed target.")
else:
    all_predictions_df = pd.DataFrame({'smiles': smiles})
    start_index = 0
    print("Starting from scratch.")

# Training and prediction for remaining targets
